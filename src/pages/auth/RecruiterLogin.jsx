...
import { Mail, Lock, Building2, Briefcase, ArrowLeft, Home } from 'lucide-react';
...
export default function RecruiterLogin() {
  ...
  const onSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      toast.success('Logged in to Recruiter Portal!');
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <Link 
          to="/" 
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-text-muted hover:text-gold transition-colors bg-surface border border-border px-3 py-1.5 rounded-full shadow-sm"
        >
      ...
      <div className="flex bg-surface-alt border border-border p-1 rounded-[8px] mb-4 text-xs font-semibold text-center">
        ...
        <div className="flex-1 py-2 rounded-[6px] bg-surface text-gold border border-border shadow-sm flex items-center justify-center gap-1">
      ...
      <div className="bg-surface border border-border rounded-[8px] p-[32px] shadow-sm">
        <div className="text-center mb-[28px]">
          <Link to="/" className="inline-flex items-center gap-[10px] group mb-[16px]">
            <div className="w-10 h-10 rounded-[8px] bg-gold/10 border border-gold/30 flex items-center justify-center text-gold">
              <Building2 size={22} />
            </div>
            <span className="serif font-semibold text-[22px] tracking-[0.01em] text-text">
              Webloom<span className="text-gold"> Recruiter</span>
            </span>
          </Link>
          <h1 className="serif text-[26px] font-semibold text-text mb-[6px]">Recruiter Portal Sign In</h1>
          <p className="text-[14px] text-text-muted">Post jobs, review candidate AI matches, and manage hiring.</p>
        </div>

        <div className="mb-[20px] p-[12px] bg-surface-alt border border-gold/30 rounded-[6px] text-xs text-text-muted">
          <strong className="text-gold">Demo Credentials:</strong><br/>
          Email: <span className="text-text font-mono">recruiter@demo.webloom.ai</span><br/>
          Password: <span className="text-text font-mono">password123</span>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-[18px]">
          <div>
            <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5">Recruiter Email</label>
            <div className="relative">
              ...
              <input 
                ...
                type="email"
                className={cn(
                  "w-full bg-surface border rounded-[6px] py-[10px] pl-[42px] pr-[16px] text-[14.5px] text-text focus:outline-none focus:border-gold focus:ring-1 focus:ring-gold transition-colors",
          ...
          >
            {isLoading ? <span className="animate-spin w-[18px] h-[18px] border-2 border-[#201607]/30 border-t-[#201607] rounded-full" /> : 'Sign In to Recruiter Portal'}
          </button>
        </form>

        <div className="mt-6 text-center text-xs text-text-muted">
          Looking for job candidate access? <Link to="/login" className="text-gold font-semibold hover:underline">Sign in as Candidate</Link>
        </div>
      </div>
    </motion.div>
  );
